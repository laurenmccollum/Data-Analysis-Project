...
#A0 = lamda N0 at t=0
#N0 is only number detected by the detector
#N0 = trueN0 * fraction
#trueN0 = N0/fraction
#N0 = A0/lamda

#actually might need to take into account the isotropic decay therefore use volume of sphere
#why would that be relevant?
#sphere volume * detector cross section * what??? 

#need to include all the assumptions made in the commments

#from medical lecture:
#area detector/ area sphere = csa/ 4pi*(distance**2) = fraction of photons emitted which are collected by the detector
#then he does an intensity equation which i assume cannot be relevant, maybe an assumption could be a constant intensity
#count rate = fraction of photons emitted which are collected by the detector * activity of the source (this gives in units photons/second aka not what we want)
#another assumption could be a 100% efficient detector

#detector cross section = 3.142 +/- 0.001 cm^2
crosssection = 3.142e-4
crosserror = 0.001e-4

d = np.array([0.0500, 0.1000, 0.1800,0.3000])

sphere1 = (4*np.pi)/(d[0]**2)
sphere2 = (4*np.pi)/(d[1]**2)
sphere3 = (4*np.pi)/(d[2]**2)
sphere4 = (4*np.pi)/(d[3]**2)

fraction1 = crosssection/sphere1
fraction2 = crosssection/sphere2
fraction3 = crosssection/sphere3
fraction4 = crosssection/sphere4

N01 = (A0[0]*halflife1)/0.693
N02 = (A0[1]*halflife2)/0.693
N03 = (A0[2]*halflife3)/0.693
N04 = (A0[3]*halflife4)/0.693

trueN01 = N01/fraction1
trueN02 = N01/fraction2
trueN03 = N01/fraction3
trueN04 = N01/fraction4

print(trueN01*10**-9,trueN02*10**-9,trueN03*10**-9,trueN04*10**-9)

#part 5
def radioactivedecay(t,decayconstant,A0):
    return np.array(A0*np.exp(-decayconstant*t))
...
#go through and check that the sign of the decay constant isnt messed up, should be positive?
#if its negative it will mess up some equations i think
